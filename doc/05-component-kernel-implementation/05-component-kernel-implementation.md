...
I chose a flat row-major `double[]` array as the representation for `Tensor1`.
The design stores element `(i, j)` at `data[i * cols + j]` using three fields:
`double[] data`, `int rows`, and `int cols`.

The primary motivation is efficiency: `get` and `set` are O(1) single-array-index
operations, and `reshape` is O(1) because only the `rows` and `cols` integers
change: the data array is never copied or reordered. This makes `reshape`
dramatically cheaper than a representation using a 2D `double[][]`, where any
shape change would require allocation and element-by-element copying.

A secondary motivation is simplicity: the representation exactly uses the principle from the
proof-of-concept, which was already validated to work correctly.
There are no internal classes, no aliasing concerns, and the convention and
correspondence are straightforward to state precisely:

- **Convention**: `data != null`, `rows > 0`, `cols > 0`,
  `data.length == rows * cols`, element `(i, j)` lives at
  `data[i * cols + j]`
- **Correspondence**: `this` is the `rows`-by-`cols` matrix whose entry at
  `(i, j)` is `data[i * cols + j]`
